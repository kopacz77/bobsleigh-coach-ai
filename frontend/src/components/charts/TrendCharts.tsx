...
export function TrendCharts({ feedbackHistory, timeRange = '14d', onTimeRangeChange }: TrendChartsProps) {
        ...
        <Grid>
          <Grid.Col span={6}>
            <Card padding="sm" withBorder>
              <Group justify="space-between" align="center">
                <Stack gap={2}>
                  <Text size="xs" c="dimmed">RPE Trend</Text>
                  <Group gap="xs" align="center">
                    {getTrendIcon(trendAnalysis.rpe)}
                    <Text size="sm" fw={500}>
                      {trendAnalysis.rpe > 0 ? '+' : ''}{trendAnalysis.rpe.toFixed(1)}
                    </Text>
                  </Group>
                </Stack>
                <Badge 
                  color={getTrendColor(trendAnalysis.rpe, true)} 
                  variant="light" 
                  size="sm"
                >
                  {Math.abs(trendAnalysis.rpe) < 0.5 ? 'Stable' : 
                   trendAnalysis.rpe > 0 ? 'Increasing' : 'Decreasing'}
                </Badge>
              </Group>
            </Card>
          </Grid.Col>
          ...
          <Grid.Col span={6}>
            <Card padding="sm" withBorder>
              <Group justify="space-between" align="center">
                <Stack gap={2}>
                  <Text size="xs" c="dimmed">Energy Trend</Text>
                  <Group gap="xs" align="center">
                    {getTrendIcon(trendAnalysis.energy)}
                    <Text size="sm" fw={500}>
                      {trendAnalysis.energy > 0 ? '+' : ''}{trendAnalysis.energy.toFixed(1)}
                    </Text>
                  </Group>
                </Stack>
                <Badge 
                  color={getTrendColor(trendAnalysis.energy)} 
                  variant="light" 
                  size="sm"
                >
                  {Math.abs(trendAnalysis.energy) < 0.5 ? 'Stable' : 
                   trendAnalysis.energy > 0 ? 'Improving' : 'Declining'}
                </Badge>
              </Group>
            </Card>
          </Grid.Col>
