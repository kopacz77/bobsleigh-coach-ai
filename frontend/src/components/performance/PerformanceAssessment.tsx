...
const PerformanceAssessment: React.FC<PerformanceAssessmentProps> = ({ userId }) => {
  ...
  const testTypesByCategory: TestTypesByCategory = {
    ...
    power: [
      ...
      { value: 'power_clean_80pct', label: 'Power Clean @ 80% (Power)', unit: 'watts', target: 'higher' },
    ...
    technique: [
      { value: 'push_technique_score', label: 'Push Technique Score', unit: 'points', target: 'higher' },
      { value: 'loading_efficiency', label: 'Loading Efficiency', unit: 'points', target: 'higher' },
      { value: 'driving_line_accuracy', label: 'Driving Line Accuracy', unit: 'points', target: 'higher' },
      { value: 'turn_execution', label: 'Turn Execution Quality', unit: 'points', target: 'higher' },
      { value: 'start_coordination', label: 'Start Coordination', unit: 'points', target: 'higher' }
    ]
  };
  ...
  return (
    <Box>
      <Title order={2} mb="md">Performance Assessment</Title>
      <Text color="dimmed" mb="xl">
        Track and analyze your performance metrics to measure progress and identify areas for improvement.
      </Text>
              ...
              <>
                ...
                <Box mb="md">
                  ...
                  <Slider
                    ...
                    color={assessment.rating <= 3 ? 'red' : 
                           assessment.rating <= 6 ? 'yellow' : 'green'}
                  />
                </Box>
                    ...
                    {assessments.slice(0, 5).map((item) => {
                      ...
                      return (
                        <List.Item
                          key={item.id}
                          icon={(
                            <ThemeIcon color={item.rating <= 3 ? 'red' : 
                                       item.rating <= 6 ? 'yellow' : 'green'} 
                                     size={24} 
                                     radius="xl"
                            >
                              {item.rating}
